'''
*@BelongsProject: End_of_term_revision
*@BelongsPackage: 
*@Author: 程序员Eighteen
*@CreateTime: 2025-12-17  21:29
*@Description: 统一设置提示词
*@Version: 1.0
'''

# 文本解析为题目 JSON 的提示词
PARSE_TEXT_TO_QUESTIONS_PROMPT = '''你是一个专业的答案解析专家。请将以下文本解析成标准的题目格式。

文本内容：
{text}

科目：{subject}

请严格按照以下 JSON 格式输出（必须是有效的 JSON）：
{{
  "subject": "{subject}",
  "questions": [
    {{
      "type": "single/multiple/judge/fill/major",
      "question": "题目内容",
      "options": ["A. 选项1", "B. 选项2", "C. 选项3", "D. 选项4"],
      "answer": "正确答案（单选填A/B/C/D，多选填A,B,C，判断填对/错，填空题填具体答案）",
      "analysis": "答案解析"
    }}
  ]
}}

注意事项：
1. type 只能是：single（单选）、multiple（多选）、judge（判断）、fill（填空题）、major（大型题）
2. 判断题的 options 为 ["对", "错"]，答案只能是 "对" 或 "错"
3. 填空题的 options 可以为空数组 []
4. 如果文本中没有答案，请根据题目内容生成正确答案
5. 如果文本中没有解析，请根据题目内容生成详细解析，并写出关键计算步骤
6. 必须返回有效的 JSON 格式，不要包含任何其他文字
7. 必须从文本中尽量识别出所有题目，禁止返回 "questions": []
8. 文本通常为试卷或复习题，带有编号 1.、2.、(1)、(2) 等，请根据这些编号拆分成多道题目。遇到类似“19. 判断题”这种小题集合时，(1)、(2)… 每一小问都要单独输出为一条 type="judge" 的题目
9. 当题干中出现“_____”“（ ）”等空格，需要填写一个或多个数值、概率、函数表达式等明确答案时，即使需要一定计算步骤，也统一使用填空题 type="fill"
10. 只有当题目要求书写较长的推导过程、证明、论述或综合分析时，才使用大型题 type="major"，options 使用 []
11. 大型题必须是主观题，analysis 必须给出分步推导过程（列公式、代入、计算每一步），而不是只给出最终答案
12. 对于 single/multiple 题，options 中每一项必须包含完整的选项内容，禁止出现只有 "A."、"B."、"C."、"D." 而没有文字或公式的情况

**【数学/物理公式识别要求】**
11. 所有数学公式和物理公式必须使用 LaTeX 格式：
   - 行内公式使用 $...$ 包裹，如：已知函数 $f(x)=x^2+2x+1$
   - 独立公式使用 $$...$$ 包裹，如：$$\\int_0^1 x^2 dx = \\frac{{1}}{{3}}$$
8. 必须保留完整的 LaTeX 语法：
   - 分式：\\frac{{a}}{{b}}
   - 上下标：x^2, x_i
   - 希腊字母：\\alpha, \\beta, \\gamma, \\theta, \\pi
   - 向量：\\vec{{v}}, \\overrightarrow{{AB}}
   - 积分：\\int, \\sum, \\lim
   - 根号：\\sqrt{{x}}, \\sqrt[n]{{x}}
9. 禁止将公式转换为自然语言描述
10. 禁止使用 MathML、图片或 Unicode 拼凑符号代替 LaTeX
'''

# OCR 文本解析为题目 JSON 的提示词
PARSE_IMAGE_TO_QUESTIONS_PROMPT = '''你是一个专业的题目识别专家。以下是从图片中 OCR 提取的文本，可能包含题目、选项等信息。

OCR 文本：
{image_text}

科目：{subject}

请识别并解析出题目，严格按照以下 JSON 格式输出（必须是有效的 JSON）：
{{
  "subject": "{subject}",
  "questions": [
    {{
      "type": "single/multiple/judge/fill/major",
      "question": "题目内容",
      "options": ["A. 选项1", "B. 选项2", "C. 选项3", "D. 选项4"],
      "answer": "正确答案",
      "analysis": "答案解析"
    }}
  ]
}}

注意事项：
1. type 只能是：single（单选）、multiple（多选）、judge（判断）、fill（填空题）、major（大型题）
2. 判断题的 options 为 ["对", "错"]，答案只能是 "对" 或 "错"
3. 填空题的 options 可以为空数组 []
4. 如果图片中没有标注答案，请根据题目内容生成正确答案
5. 必须生成详细的答案解析，并写出关键计算步骤
6. 必须返回有效的 JSON 格式，不要包含任何其他文字
7. 必须从 OCR 文本中尽量识别出所有题目，禁止返回 "questions": []
8. OCR 文本通常包含编号 1.、2.、(1)、(2) 等，请根据这些编号拆分成多道题目。遇到类似“19. 判断题”这种小题集合时，(1)、(2)… 每一小问都要单独输出为一条 type="judge" 的题目
9. 当题干中出现“_____”“（ ）”等空格，需要填写一个或多个数值、概率、函数表达式等明确答案时，即使需要一定计算步骤，也统一使用填空题 type="fill"
10. 只有当题目要求书写较长的推导过程、证明、论述或综合分析时，才使用大型题 type="major"，options 使用 []
11. 大型题必须是主观题，analysis 必须给出分步推导过程（列公式、代入、计算每一步），而不是只给出最终答案
12. 对于 single/multiple 题，options 中每一项必须包含完整的选项内容，禁止出现只有 "A."、"B."、"C."、"D." 而没有文字或公式的情况

**【数学/物理公式识别要求】**
11. 所有数学公式和物理公式必须使用 LaTeX 格式：
   - 行内公式使用 $...$ 包裹，如：已知函数 $f(x)=x^2+2x+1$
   - 独立公式使用 $$...$$ 包裹，如：$$\\int_0^1 x^2 dx = \\frac{{1}}{{3}}$$
8. 必须保留完整的 LaTeX 语法：
   - 分式：\\frac{{a}}{{b}}
   - 上下标：x^2, x_i
   - 希腊字母：\\alpha, \\beta, \\gamma, \\theta, \\pi
   - 向量：\\vec{{v}}, \\overrightarrow{{AB}}
   - 积分：\\int, \\sum, \\lim
   - 根号：\\sqrt{{x}}, \\sqrt[n]{{x}}
9. 禁止将公式转换为自然语言描述
10. 禁止使用 MathML、图片或 Unicode 拼凑符号代替 LaTeX
'''

# 图片直接解析为题目 JSON 的提示词
PARSE_IMAGE_DIRECT_PROMPT = '''你是一个专业的题目识别专家。请仔细观察图片中的题目内容，识别并解析出所有题目。

科目：{subject}

请严格按照以下 JSON 格式输出（必须是有效的 JSON）：
{{
  "subject": "{subject}",
  "questions": [
    {{
      "type": "single/multiple/judge/fill/major",
      "question": "题目内容",
      "options": ["A. 选项1", "B. 选项2", "C. 选项3", "D. 选项4"],
      "answer": "正确答案（单选填A/B/C/D，多选填A,B,C，判断填对/错，填空题填具体答案）",
      "analysis": "答案解析"
    }}
  ]
}}

注意事项：
1. 仔细识别题目类型：single（单选）、multiple（多选）、judge（判断）、fill（填空题）、major（大型题）
2. 判断题的 options 为 ["对", "错"]，答案只能是 "对" 或 "错"
3. 填空题的 options 可以为空数组 []
4. 如果图片中包含答案，请使用图片中的答案；如果没有答案，请根据题目内容生成正确答案
5. 必须生成详细的答案解析，说明为什么这个答案是正确的，并写出关键计算步骤
6. 必须返回有效的 JSON 格式，不要包含任何其他文字
7. 确保题目内容、选项、答案完整准确，不要出现乱码或遗漏
8. 必须尽量识别出所有题目，禁止返回 "questions": []
9. 试卷通常带有题号 1.、2.、(1)、(2) 等，请依据题号拆分成多道题目。遇到类似“19. 判断题”这种小题集合时，(1)、(2)… 每一小问都要单独输出为一条 type="judge" 的题目
10. 当题干中出现“_____”“（ ）”等空格，需要填写一个或多个数值、概率、函数表达式等明确答案时，即使需要一定计算步骤，也统一使用填空题 type="fill"
11. 只有当题目要求书写较长的推导过程、证明、论述或综合分析时，才使用大型题 type="major"，options 使用 []
12. 对于 single/multiple 题，options 中每一项必须包含完整的选项内容，禁止出现只有 "A."、"B."、"C."、"D." 而没有文字或公式的情况

**【数学/物理公式识别要求】**
12. 所有数学公式和物理公式必须使用 LaTeX 格式：
   - 行内公式使用 $...$ 包裹，如：已知函数 $f(x)=x^2+2x+1$
   - 独立公式使用 $$...$$ 包裹，如：$$\\int_0^1 x^2 dx = \\frac{{1}}{{3}}$$
13. 必须保留完整的 LaTeX 语法：
   - 分式：\\frac{{a}}{{b}}
   - 上下标：x^2, x_i
   - 希腊字母：\\alpha, \\beta, \\gamma, \\theta, \\pi
   - 向量：\\vec{{v}}, \\overrightarrow{{AB}}
   - 积分：\\int, \\sum, \\lim
   - 根号：\\sqrt{{x}}, \\sqrt[n]{{x}}
   - 物理单位：m/s, kg·m/s^2（单位不需要公式符号）
14. 禁止将公式转换为自然语言描述
15. 禁止使用 MathML、图片或 Unicode 拼凑符号代替 LaTeX
'''

# 生成答案解析的提示词
GENERATE_ANALYSIS_PROMPT = '''请为以下题目生成详细的解析说明。

题目：{question}

选项：
{options_text}

正确答案：{answer}

请生成详细的解析，说明为什么这个答案是正确的，以及解题思路。
'''

# 系统角色提示词
SYSTEM_ROLE_PARSE_TEXT = "你是一个专业的答案解析助手，擅长将文本转换为结构化的题目数据。"
SYSTEM_ROLE_PARSE_IMAGE = "你是一个专业的图片题目识别助手，擅长从 OCR 文本中提取并结构化题目数据。"
SYSTEM_ROLE_GENERATE_ANALYSIS = "你是一个专业的教育专家，擅长为题目提供清晰易懂的解析。"

# 结构化答案解析提示词（新增）
PARSE_STRUCTURED_QUESTIONS_PROMPT = '''你是题目处理专家。我已经使用正则表达式预处理了文档，提取了题号和题型信息。
现在请你为每道题补充正确答案、详细解析，并在必要时修正题型。

预处理结果:
{structured_data}

科目：{subject}

请严格按照以下 JSON 格式输出（必须是有效的 JSON）：
{{
  "subject": "{subject}",
  "questions": [
    {{
      "type": "题型（single/multiple/judge/fill/major，可修正预处理结果）",
      "question": "题干内容",
      "options": ["选项数组"],
      "answer": "正确答案",
      "analysis": "详细解析"
    }}
  ]
}}

注意事项：
1. **题型处理规则**:
   - 如果预处理检测到的题型置信度 >= 0.85，优先使用该题型
   - 如果置信度 < 0.85 或为 unknown，请根据题目内容确定正确题型
   - type 只能是：single（单选）、multiple（多选）、judge（判断）、fill（填空题）、major（大型题）

2. **选项处理规则**:
   - 判断题的 options 必须为 ["对", "错"]，答案只能是 "对" 或 "错"
   - 填空题和大型题的 options 使用空数组 []
   - 单选/多选题必须包含完整选项，禁止出现只有 "A."、"B." 而没有内容的情况

3. **答案生成规则**:
   - 如果原文中包含答案，请使用原文答案
   - 如果原文中没有答案，请根据题目内容生成正确答案
   - 单选题答案格式：A 或 B 或 C 或 D
   - 多选题答案格式：A,B,C（按字母顺序，用英文逗号分隔）
   - 判断题答案格式：对 或 错
   - 填空题答案：具体的填空题内容（可能包含数学公式）

4. **解析生成规则**:
   - 必须生成详细的答案解析，说明为什么这个答案是正确的
   - 对于计算题/大题，必须写出关键计算步骤和推导过程
   - 解析要清晰易懂，适合学生复习使用

5. **数学公式处理**:
   - 所有数学公式和物理公式必须使用 LaTeX 格式
   - 行内公式使用 $...$ 包裹，如：$f(x)=x^2+2x+1$
   - 独立公式使用 $$...$$ 包裹，如：$$\\int_0^1 x^2 dx = \\frac{{1}}{{3}}$$
   - 禁止将公式转换为自然语言描述

6. **输出要求**:
   - 必须返回有效的 JSON 格式，不要包含任何其他文字
   - 确保所有题目都被处理，不要遗漏

7. **特殊条目处理**:
   - 如果遇到 content 仅包含 "判断题"、"选择题"、"填空题" 等标题的条目，这是大题标题，请忽略该条目，不要输出到结果中。
   - 必须处理该标题下方的所有小题（通常紧随其后）。
'''

SYSTEM_ROLE_PARSE_STRUCTURED = "你是一个专业的题目处理助手，擅长为预处理的题目补充答案和解析。"


# ==================== 资料库AI题目生成提示词 ====================

# 从资料生成单选题的提示词
GENERATE_SINGLE_CHOICE_FROM_MATERIAL = '''请根据以下{subject_name}学习资料，生成{count}道单选题。

学习资料：
{text}

要求：
1. 题目应该测试对关键概念的理解
2. 每道题提供4个选项（A、B、C、D）
3. 只有一个正确答案
4. 提供详细的解析说明
5. 题目难度适中，适合学生练习

**【数学/物理公式要求】**
- 所有数学公式和物理公式必须使用 LaTeX 格式
- 行内公式使用 $...$ 包裹，如：$f(x)=x^2+2x+1$
- 独立公式使用 $$...$$ 包裹，如：$$\\int_0^1 x^2 dx = \\frac{{1}}{{3}}$$
- 必须保留完整的 LaTeX 语法（分式、上下标、希腊字母、积分、根号等）

请以JSON格式返回，格式如下：
[
  {{
    "question": "题干内容",
    "options": {{"A": "选项A", "B": "选项B", "C": "选项C", "D": "选项D"}},
    "answer": "A",
    "analysis": "解析内容"
  }}
]

只返回JSON数组，不要其他内容。'''

# 从资料生成多选题的提示词
GENERATE_MULTIPLE_CHOICE_FROM_MATERIAL = '''请根据以下{subject_name}学习资料，生成{count}道多选题。

学习资料：
{text}

要求：
1. 题目应该测试对知识点的综合理解
2. 每道题提供4-5个选项（A、B、C、D、E）
3. 有2个或以上正确答案
4. 提供详细的解析说明
5. 题目难度适中

**【数学/物理公式要求】**
- 所有数学公式和物理公式必须使用 LaTeX 格式
- 行内公式使用 $...$ 包裹，如：$f(x)=x^2+2x+1$
- 独立公式使用 $$...$$ 包裹，如：$$\\int_0^1 x^2 dx = \\frac{{1}}{{3}}$$
- 必须保留完整的 LaTeX 语法（分式、上下标、希腊字母、积分、根号等）

请以JSON格式返回，格式如下：
[
  {{
    "question": "题干内容",
    "options": {{"A": "选项A", "B": "选项B", "C": "选项C", "D": "选项D"}},
    "answer": "AB",
    "analysis": "解析内容"
  }}
]

只返回JSON数组，不要其他内容。'''

# 从资料生成判断题的提示词
GENERATE_JUDGE_FROM_MATERIAL = '''请根据以下{subject_name}学习资料，生成{count}道判断题。

学习资料：
{text}

要求：
1. 题目应该测试对概念的准确理解
2. 答案为"正确"或"错误"
3. 提供详细的解析说明
4. 避免模棱两可的表述

**【数学/物理公式要求】**
- 所有数学公式和物理公式必须使用 LaTeX 格式
- 行内公式使用 $...$ 包裹，如：$f(x)=x^2+2x+1$
- 独立公式使用 $$...$$ 包裹，如：$$\\int_0^1 x^2 dx = \\frac{{1}}{{3}}$$

请以JSON格式返回，格式如下：
[
  {{
    "question": "题干内容",
    "answer": "正确",
    "analysis": "解析内容"
  }}
]

只返回JSON数组，不要其他内容。'''

# 从资料生成填空题的提示词
GENERATE_FILL_FROM_MATERIAL = '''请根据以下{subject_name}学习资料，生成{count}道填空题。

学习资料：
{text}

要求：
1. 题目应该测试对关键概念和术语的记忆
2. 用______表示空白处
3. 提供标准答案
4. 提供详细的解析说明

**【数学/物理公式要求】**
- 所有数学公式和物理公式必须使用 LaTeX 格式
- 行内公式使用 $...$ 包裹，如：$f(x)=x^2+2x+1$
- 独立公式使用 $$...$$ 包裹，如：$$\\int_0^1 x^2 dx = \\frac{{1}}{{3}}$$

请以JSON格式返回，格式如下：
[
  {{
    "question": "题干内容，包含______空白处",
    "answer": "标准答案",
    "analysis": "解析内容"
  }}
]

只返回JSON数组，不要其他内容。'''

# 从资料生成大题的提示词
GENERATE_MAJOR_FROM_MATERIAL = '''请根据以下{subject_name}学习资料，生成{count}道大题（简答题或计算题）。

学习资料：
{text}

要求：
1. 题目应该测试对知识的综合应用能力
2. 需要详细的解答过程
3. 提供完整的答案和解析
4. 题目有一定深度

**【数学/物理公式要求】**
- 所有数学公式和物理公式必须使用 LaTeX 格式
- 行内公式使用 $...$ 包裹，如：$f(x)=x^2+2x+1$
- 独立公式使用 $$...$$ 包裹，如：$$\\int_0^1 x^2 dx = \\frac{{1}}{{3}}$$
- 必须保留完整的 LaTeX 语法（分式、上下标、希腊字母、积分、根号等）

请以JSON格式返回，格式如下：
[
  {{
    "question": "题干内容",
    "answer": "详细答案",
    "analysis": "解析和评分要点"
  }}
]

只返回JSON数组，不要其他内容。'''

# 系统角色提示词
SYSTEM_ROLE_GENERATE_FROM_MATERIAL = "你是一个专业的教育专家，擅长根据学习资料生成高质量的练习题目。"
