...
from database.models import ExamSchedule, Subject
...
class ExamScheduleCreate(BaseModel):
    ...
    exam_time: str          # ISO 8601 字符串，如 "2026-07-15T09:00:00"
...
@router.get("")
def list_exams(user_id: int, db: Session = Depends(get_default_db)):
    """获取该用户的所有考试日程（按考试时间升序）"""
    exams = (
        db.query(ExamSchedule)
        .filter(ExamSchedule.user_id == user_id)
        .order_by(ExamSchedule.exam_time.asc())
        .all()
    )
    return {
        "code": 200,
        "message": "获取成功",
        "data": [_to_dict(e) for e in exams]
    }


@router.get("/upcoming")
def get_upcoming_exam(user_id: int, db: Session = Depends(get_default_db)):
    """
    获取下一门考试（考试时间 > 当前时间，取最近的一条）
    前端顶栏倒计时专用
    """
    now = datetime.now()
    exam = (
    ...
    return {
        "code": 200,
        "message": "获取成功",
        "data": _to_dict(exam) if exam else None
    }
